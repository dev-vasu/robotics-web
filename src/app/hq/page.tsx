  ...
  if (!isAuthenticated) {
    return (
      <main className="min-h-screen bg-transparent grid-bg flex flex-col items-center justify-center p-6 relative">
        ...
        <div className="w-full max-w-md glass-panel border-4 border-cyber-blue/30 bg-black/90 p-8 text-center relative z-10">
          ...
          <form onSubmit={(e) => { e.preventDefault(); fetchRecords(passcode); }} className="space-y-4">
            <input type="password" placeholder="ENTER_ADMIN_PASSCODE..." value={passcode} onChange={(e) => setPasscode(e.target.value)} className="w-full bg-white/5 border-2 border-white/10 px-4 py-3 text-center text-white font-mono focus:outline-none focus:border-hyper-pink" />
            <button type="submit" className="w-full py-4 bg-hyper-pink text-black font-black uppercase tracking-widest hover:bg-white transition-all shadow-[8px_8px_0_0_#00f0ff]">{isLoadingRecords ? "AUTHENTICATING..." : "ACCESS_CORE"}</button>
          </form>
        </div>
      </main>
    );
    ...
    <main className="min-h-screen bg-transparent grid-bg flex flex-col pt-20 px-6 pb-20 relative overflow-x-hidden" ref={containerRef}>
      ...
      <div className="container mx-auto max-w-[1600px] flex flex-col gap-10 relative z-10">
        <div className="flex flex-wrap gap-4 border-b-4 border-white/5 pb-8">
           {SECTORS.map(s => (
             <button key={s.id} onClick={() => { setActiveSector(s.id); setActiveTab(s.sub[0]); setSelectedEmail(null); }} className={`flex items-center gap-4 px-8 py-5 border-2 transition-all group ${activeSector === s.id ? "bg-white text-black border-white shadow-[0_0_30px_rgba(255,255,255,0.2)]" : "bg-black/80 text-white/40 border-white/10 hover:border-hyper-pink hover:text-white"}`}><s.icon className={`w-6 h-6 transition-transform group-hover:scale-125 ${activeSector === s.id ? "text-hyper-pink" : ""}`} /><span className="text-xl font-black italic uppercase tracking-tighter">{s.label}</span></button>
           ))}
        </div>

        <div className="grid lg:grid-cols-12 gap-10">
          <div className="lg:col-span-4 flex flex-col gap-10 hq-panel">
            <div className="glass-panel border-4 border-cyber-blue/30 bg-black/80 p-8 relative overflow-hidden group">
              ...
              <div className="flex items-center justify-between mb-8 border-b border-white/10 pb-6">
                <div className="flex items-center gap-4"><div className="p-3 bg-cyber-blue rounded-full"><Send className="w-6 h-6 text-black" /></div><div><h2 className="text-3xl font-black italic text-white uppercase tracking-tighter leading-none">TRANSMITTER</h2><p className="text-cyber-blue font-black uppercase tracking-[0.4em] text-[10px] mt-1">SECURE_UPLINK</p></div></div>
                {activeTab === "SQUAD" && <button onClick={handleSquadBlast} className="px-4 py-2 bg-[#ccff00] text-black font-black text-[10px] uppercase tracking-widest hover:bg-white transition-all shadow-[4px_4px_0_0_#ff007a]">SQUAD_BLAST</button>}
              </div>
              <form onSubmit={handleSend} className="space-y-6">
                <div className="space-y-4">
                  <div><label className="block text-[10px] font-black uppercase tracking-widest text-white/40 mb-2">TARGET_NODE</label><input type="email" placeholder="CLIENT@DOMAIN.COM" value={to} onChange={(e) => setTo(e.target.value)} className="w-full bg-white/5 border-2 border-white/10 px-4 py-3 text-white font-mono focus:outline-none focus:border-cyber-blue transition-colors" /></div>
                  <div><label className="block text-[10px] font-black uppercase tracking-widest text-white/40 mb-2">SUBJECT</label><input type="text" placeholder="RE: YOUR INQUIRY" value={subject} onChange={(e) => setSubject(e.target.value)} className="w-full bg-white/5 border-2 border-white/10 px-4 py-3 text-white font-mono focus:outline-none focus:border-cyber-blue transition-colors" /></div>
                  <div>
                    <div className="flex justify-between items-center mb-2"><label className="block text-[10px] font-black uppercase tracking-widest text-white/40">DATA_PAYLOAD</label><div className="flex gap-2">{EMAIL_TEMPLATES.map(temp => (
                      <button key={temp.id} type="button" onClick={() => { setMessage(temp.msg); setSubject(temp.subj); }} className="text-[8px] font-black border border-white/10 px-2 py-1 hover:bg-hyper-pink hover:text-white transition-all uppercase rounded-sm bg-white/5">{temp.label}</button>
                    ))}</div></div>
                ...
                <button type="submit" disabled={status === "loading"} className="w-full py-6 bg-cyber-blue text-black font-black text-xl uppercase italic hover:bg-white transition-all shadow-[10px_10px_0_0_#ff007a] disabled:opacity-50">{status === "loading" ? "ENCRYPTING..." : "TRANSMIT_SIGNAL"}</button>
              </form>
            </div>
            <div className="grid grid-cols-2 gap-4 hq-panel">
               <div className="glass-panel p-6 border-l-4 border-cyber-blue bg-black/80 flex flex-col justify-between"><div className="text-[10px] font-black text-white/40 uppercase mb-2 flex items-center gap-2"><Users className="w-3 h-3" /> SQUAD</div><div className="text-4xl font-black italic text-white animate-pulse">{squadCount}</div></div>
               <button onClick={handleExportData} className="glass-panel p-6 border-l-4 border-[#ffaa00] bg-black/80 hover:bg-white/5 transition-all text-left flex flex-col justify-between group"><div className="text-[10px] font-black text-white/40 uppercase mb-2">BACKUP</div><div className="flex items-center gap-2 text-2xl font-black italic text-[#ffaa00] group-hover:translate-x-2 transition-transform"><Download className="w-5 h-5" /> EXPORT</div></button>
            </div>
          </div>

          <div className="lg:col-span-8 glass-panel border-4 border-hyper-pink/30 bg-black/80 p-8 flex flex-col hq-panel" style={{ height: "800px" }}>
            <div className="flex items-center justify-between mb-8 border-b border-white/10 pb-6 shrink-0">
              <div className="flex items-center gap-4"><div className="p-3 bg-hyper-pink rounded-full shadow-[0_0_15px_#ff007a] animate-pulse"><Database className="w-6 h-6 text-black" /></div><div><h2 className="text-3xl font-black italic text-white uppercase tracking-tighter leading-none">{activeSector}</h2><p className="text-hyper-pink font-black uppercase tracking-[0.4em] text-[10px] mt-1">CORE_INTELLIGENCE_ACTIVE</p></div></div>
              <div className="flex items-center gap-4">{selectedEmail && <button onClick={() => { setSelectedEmail(null); setTo(""); setSubject(""); }} className="px-6 py-3 bg-white/5 border border-white/10 hover:bg-hyper-pink hover:text-black transition-all text-white rounded text-[10px] font-black tracking-widest uppercase shadow-[4px_4px_0_0_white]">CLOSE_THREAD</button>}<button onClick={() => fetchRecords(passcode)} className="p-4 bg-white/5 border border-white/10 hover:bg-hyper-pink hover:text-black transition-all text-white rounded"><RefreshCcw className={`w-6 h-6 ${isLoadingRecords ? "animate-spin" : ""}`} /></button></div>
            </div>

            {!selectedEmail && (
              <div className="flex gap-4 mb-8 bg-white/5 p-3 border border-white/10 rounded-xl shrink-0 overflow-x-auto">
                {SECTORS.find(s => s.id === activeSector)?.sub.map(tab => (
                  <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-1 py-4 px-6 text-[10px] font-black tracking-[0.2em] uppercase transition-all rounded-lg whitespace-nowrap ${activeTab === tab ? "bg-hyper-pink text-black shadow-[0_0_20px_#ff007a]" : "text-white/30 hover:bg-white/10 hover:text-white"}`}>{tab.replace('_', ' ')}</button>
                ))}
              </div>
            )}
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar pr-4">
              {activeTab === "GLOBAL_ALERT" ? (
                <div className="p-10 space-y-8 animate-in fade-in zoom-in duration-500"><div className="flex items-center justify-between mb-4 border-b border-hyper-pink/20 pb-4"><div className="flex items-center gap-4 text-hyper-pink"><Radio className="w-12 h-12 animate-pulse" /><h3 className="text-5xl font-black italic uppercase tracking-tighter">LIVE_BROADCAST</h3></div><div className="flex gap-2">{BROADCAST_TEMPLATES.map(bt => (<button key={bt.label} onClick={() => setActiveBroadcast(bt.msg)} className="text-[8px] font-black border border-white/10 px-3 py-1.5 hover:bg-hyper-pink hover:text-black transition-all uppercase bg-white/5 rounded-sm">{bt.label}</button>))}</div></div><div className="space-y-6"><textarea value={activeBroadcast} onChange={(e) => setActiveBroadcast(e.target.value)} placeholder="ENTER_GLOBAL_MESSAGE..." className="w-full bg-black border-2 border-white/10 p-8 text-2xl text-white font-mono focus:border-hyper-pink focus:outline-none h-60 placeholder:text-white/5" /><button onClick={handleUpdateBroadcast} className="w-full py-8 bg-hyper-pink text-black font-black text-3xl uppercase italic hover:bg-white transition-all shadow-[15px_15px_0_0_white] active:translate-y-1 active:shadow-none">ACTIVATE_GLOBAL_UPLINK</button></div></div>
                ...
                <div className="grid md:grid-cols-2 gap-4 animate-in fade-in slide-in-from-right-5 duration-500">{["strike", "beat", "typer", "run", "pong", "snake", "brick", "stacks", "maze", "dodge", "jump", "canvas", "beats", "terminal"].map(id => {
                  ...
                  return ( <div key={id} className="flex items-center justify-between p-8 bg-black/50 border-2 border-white/5 hover:border-hyper-pink transition-all group relative overflow-hidden"><div className="absolute top-0 left-0 w-1 h-full bg-white/10 group-hover:bg-hyper-pink transition-colors" /><div><div className="text-[8px] font-black text-white/30 uppercase mb-1">MODULE_ID</div><div className="text-2xl font-black italic text-white uppercase group-hover:text-hyper-pink transition-colors">{id}</div></div><button onClick={() => handleToggleFeature(id, isEnabled)} className={`px-8 py-3 font-black uppercase italic transition-all shadow-[5px_5px_0_0_black] hover:shadow-none active:translate-x-1 active:translate-y-1 ${isEnabled ? "bg-electric-volt text-black" : "bg-red-500 text-white"}`}>{isEnabled ? "ACTIVE" : "OFFLINE"}</button></div> );
                })}</div>
              ) : activeTab === "LEADERBOARDS" ? (
                <div className="space-y-12 animate-in fade-in zoom-in duration-500">{Array.from(new Set(leaderboardScores.map(s => s.game_id))).map(gameId => (<div key={gameId} className="space-y-4"><div className="flex items-center gap-3 border-b border-cyber-blue/30 pb-3"><Trophy className="w-6 h-6 text-cyber-blue" /><h3 className="text-2xl font-black italic text-white uppercase tracking-wider">{gameId}</h3></div><table className="w-full text-left border-collapse"><thead><tr className="border-b border-white/10 text-[10px] uppercase text-white/40"><th className="pb-3 px-6 font-black">ALIAS_ID</th><th className="pb-3 px-6 font-black">DATA_POINTS</th><th className="pb-3 px-6 font-black text-right">ACTION</th></tr></thead><tbody>{leaderboardScores.filter(s => s.game_id === gameId).map(s => (<tr key={s.id} className="border-b border-white/5 hover:bg-white/5 transition-all"><td className="py-4 px-6 font-black text-white text-lg tracking-widest">{s.initials}</td><td className="py-4 px-6 font-black italic text-electric-volt text-2xl tabular-nums">{s.score}</td><td className="py-4 px-6 text-right"><button onClick={() => handleDeleteScore(s.id)} className="p-2 text-red-500 hover:bg-red-500 hover:text-white transition-all rounded border border-transparent hover:border-white/20"><ShieldAlert className="w-6 h-6" /></button></td></tr>))}</tbody></table></div>))}{leaderboardScores.length === 0 && <div className="text-center text-white/20 font-black uppercase tracking-[0.5em] py-20 italic">NO_SIMULATION_DATA_DETECTED</div>}</div>
              ) : !selectedEmail ? (
                <div className="animate-in fade-in slide-in-from-bottom-5 duration-500"><table className="w-full text-left border-collapse"><thead><tr className="border-b-2 border-white/10 text-[10px] uppercase text-white/40"><th className="pb-3 px-6">CATEGORY</th><th className="pb-3 px-6">PARTNER_NODE</th><th className="pb-3 px-6 text-right">TIMESTAMP</th></tr></thead><tbody>{Array.from(new Set(filteredRecords.map(r => r.email))).map(email => {
                  ...
                  return (<tr key={email} onClick={() => handleSelectConversation(email)} className="border-b border-white/5 hover:bg-white/10 transition-all cursor-pointer group"><td className="py-5 px-6 whitespace-nowrap"><span className={`inline-flex items-center px-3 py-1 bg-${c.replace('text-', '')}/10 border border-${c.replace('text-', '')}/30 ${c} text-[10px] font-black tracking-widest uppercase rounded-sm`}>{l} <span className="text-white/40 ml-2">[{thread.length}]</span></span></td><td className="py-5 px-6 font-bold text-white text-base group-hover:text-hyper-pink transition-colors">{email}</td><td className="py-5 px-6 text-[10px] text-white/40 font-mono text-right whitespace-nowrap uppercase">{new Date(latest.created_at).toLocaleString()}</td></tr>);
                })}</tbody></table></div>
              ) : (
                <div className="space-y-8 thread-view pb-10">{filteredRecords.filter(r => r.email === selectedEmail).map(r => (
                  <div key={r.id} className={`p-8 border-l-8 bg-black/60 relative overflow-hidden group ${r.type === 'RECEIVED' ? 'border-electric-volt shadow-[0_0_20px_rgba(204,255,0,0.05)]' : r.type === 'FEEDBACK' ? 'border-cyber-blue' : r.type === 'TICKET' ? 'border-[#ffaa00]' : r.type === 'NEWSLETTER' ? 'border-[#ccff00]' : 'border-cyber-blue shadow-[0_0_20px_rgba(0,240,255,0.05)]'}`}><div className="flex justify-between items-start mb-6"><span className={`text-[10px] font-black uppercase tracking-widest px-3 py-1 bg-white/5 border border-white/10 ${r.type === 'RECEIVED' ? 'text-electric-volt' : r.type === 'FEEDBACK' ? 'text-cyber-blue' : r.type === 'TICKET' ? 'text-[#ffaa00]' : r.type === 'NEWSLETTER' ? 'text-[#ccff00]' : 'text-cyber-blue'}`}>{r.type === 'RECEIVED' ? 'INCOMING_UPLINK' : r.type === 'FEEDBACK' ? 'DIAGNOSTIC_DATA' : r.type === 'TICKET' ? 'SUPPORT_TICKET' : r.type === 'NEWSLETTER' ? 'SQUAD_INIT' : 'OUTGOING_SIGNAL'}</span><span className="text-[10px] text-white/30 font-mono uppercase italic">{new Date(r.created_at).toLocaleString()}</span></div><div className="text-lg font-black text-white mb-3 italic tracking-tighter">{r.subject}</div><div className="text-base font-mono text-white/70 whitespace-pre-wrap leading-relaxed border-t border-white/5 pt-4">{r.content}</div></div>
                ))}</div>
              )}
            </div>
          </div>
